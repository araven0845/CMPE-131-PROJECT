...
import {
  ...
  getDoc,
  Firestore, // Add this import for fetching single documents
...
interface WorkoutContextProps {
  ...
  clearWorkoutHistory: () => Promise<void>; // Add this line
...
export const WorkoutContext = createContext<WorkoutContextProps>({
  ...
  deleteWorkout: async () => {},
  clearWorkoutHistory: async () => {}, // Add this line
  loading: true,
});
...
export const WorkoutProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const addRoutine = async (routineData: Omit<WorkoutRoutine, 'id' | 'createdAt'>): Promise<string | null> => {
    ...
    try {
      ...
      console.log('Routine successfully created with ID:', docRef.id);
  ...
  const deleteRoutine = async (routineId:string) => {
    ...
    try {
      ...
      // Verify deletion
      const checkDoc = await getDoc(routineDocRef);

      if (!checkDoc.exists()) {
        console.log(`Routine ${routineId} successfully deleted from Firebase`);
      } else {
        console.error(`Failed to delete routine ${routineId} - document still exists!`);
      }
    } catch (error) {
  ...
  const clearWorkoutHistory = async () => {
    if (!firebaseUser) return;
    try {
      // Get all workout history documents
      const historyColRef = collection(db, 'users', firebaseUser.uid, 'workoutHistory');
      ...
      console.log('Workout history successfully cleared');
    } catch (error) {
      console.error('Failed to clear workout history:', error);
      alert('Error: Failed to clear workout history. Please try again.');
    }
  };

  return (
    <WorkoutContext.Provider value={{ routines, workoutHistory, personalRecords, addRoutine, updateRoutine, deleteRoutine, addWorkoutToHistory, deleteWorkout, clearWorkoutHistory, loading }}>
      {children}
